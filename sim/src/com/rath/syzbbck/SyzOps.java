...
public class SyzOps {
   ...
   */
  public static final boolean getBit(final byte b, final int i) {
    if (i < 0 || i > 7) {
  ...
  public static final byte getBitRange(final byte b, final int lsb, final int msb) {
    short result = 0;
    
    for (short i = (short) lsb; i <= msb; i++) {
      result += (getBit(b, i)) ? 1 << i : 0;
    }
    
    return (byte) (result >> lsb);
  }
}
